...
//       // Add buffer to capture adjacent events
...
//     } catch (error) {
//       logger.error('Error calculating session cycles:', error);
...
//   // Getting the state data based on time frame and machine serial

//   router.get("/run-session/state/timeframe", async (req, res) => {
//     try {
//       const { startTime, endTime, machineSerial } = req.query;
...
//       if (machineSerial) {
//         query["machine.serial"] = parseInt(machineSerial);
//       }
...
//       res.json(states);
//     } catch (error) {
//       logger.error("Error fetching state messages:", error);
//       res.status(500).json({ error: "Failed to fetch state messages" });
//     }
//   });

  //   testing

  // router.get('/run-session/state/all', async (req, res) => {
  //     try {
  //       const states = await db.collection('state')
  //         .find({})
  //         .project({
  ...
  //       res.json(states);
  //     } catch (error) {
  //       logger.error('Error fetching all state data:', error);
  //       res.status(500).json({ error: 'Failed to fetch state data' });
  //     }
  //   });

//   router.get("/run-session", async (req, res) => {
//     try {
//       const { machineSerial } = req.query;
//       const query = {};
...
//           "program.items": 1,
//           "program.programNumber": 1,
//           "program.batchNumber": 1,
//           "program.accountNumber": 1,
//           "program.speed": 1,
//           "program.stations": 1,
//           operators: 1,
//           "status.code": 1,
...
//       res.json(sessions);
//     } catch (error) {
//       logger.error("Error fetching run sessions:", error);
//       res.status(500).json({ error: "Failed to fetch run sessions" });
//     }
//   });

//   router.get("/run-session/:id", async (req, res) => {
//     try {
//       const { machineSerial } = req.query;
...
//       const state = await db.collection("state").findOne(query, {
//         projection: {
//           _id: 1,
//           timestamp: 1,
//           "machine.serial": 1,
...
//       const count = await db.collection("count").findOne(query, {
//         projection: {
//           _id: 1,
//           timestamp: 1,
//           "machine.serial": 1,
...
//     } catch (error) {
//       logger.error("Error fetching run session:", error);
//       res.status(500).json({ error: "Failed to fetch run session" });
//     }
//   });
