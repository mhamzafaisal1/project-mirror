...
async function fetchGroupedAnalyticsData(db, start, end, groupBy = 'machine', options = {}) {
  const { targetSerials = [], operatorId = null, preloadedStates = null } = options;
  ...
  if (preloadedStates) {
    // Flatten preloaded state arrays from each machine
    states = Object.values(preloadedStates).flat();
  } else {
    const stateQuery = {
      timestamp: { $gte: start, $lte: end },
      "machine.serial": { $type: "int" }
